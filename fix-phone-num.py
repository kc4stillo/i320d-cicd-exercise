...
def fix_phone_num(phone_num_to_fix):
    if len(phone_num_to_fix) != 10 and phone_num_to_fix[0] != "1":
        raise ValueError(
            f'phone number must be length 10; got "{phone_num_to_fix}" which is of length {len(phone_num_to_fix)}'
        )
    if not phone_num_to_fix.isdigit():
        raise ValueError(
            f'phone number must only contain digits; got "{phone_num_to_fix}"'
        )
    if len(phone_num_to_fix) == 11:
        ...
        if phone_num_to_fix[0] != "1":
            ...
            raise ValueError(
                f'phone numbers of length 11 must begin with a 1, got "{phone_num_to_fix}"'
            )
